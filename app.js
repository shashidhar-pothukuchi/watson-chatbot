...
var once = function(input) {
    if(once.done) return;
     googleTranslate.detectLanguage(input, async function(err, detection) {
  ...
  bankingServices.getPerson(7829706, function(err, person) {
    ...
    var inputstring_original;
    if (req.body) {
      if (req.body.input) {
        inputstring_original = req.body.input.text;
        let inputstring = '';
        once(inputstring_original);
              ...
              if (err) {
                ...
                checkForLookupRequests(data, function(err, data) {
                  if (err) {
                    ...
                    googleTranslate.translate(data.output.text[0], language, async function(err, translation) {
